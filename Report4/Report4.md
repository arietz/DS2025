...
#the most common value is arouund 45-60, low values are rare, and high values are rare too. It is right skewed.
```
...
```python
### Your comment here
#  The distribution described above is right-skewed. There are a few outliers. They are on the  right side.
```
...
#mean is greater than median and the data is right-skewed.
```
...
#yes they are related. The more x increases, the more y increases.
...
y = -x * 2 + np.random.normal(loc = 0, scale = 2, size = 100)
plt.scatter(x, y, alpha = .6);
#yes they are negatively related. The higher x is, the lower y.
```
...
#Legendary pokemon tend to have higher attack stats. The plot is missing the title.
```
